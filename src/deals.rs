//! The generator: a deal index in, thirteen packed bytes out.
//!
//! This module is the crate's reason to exist, and it is deliberately kept
//! free of everything else. It has no dependencies — not even on
//! `bridge-encodings`, which [`crate::pairing`] above it needs — so the
//! unranking stays testable against digests of the real library without
//! anything else travelling with it.
//!
//! # Seeking is cheap
//!
...
//! <https://github.com/bridge-craftwork/rpdd-reader>. Do not "clean up" a
//! constant without checking them: the constants are not arbitrary, and nothing
//! about a wrong one fails loudly — it simply produces deals that are not his.
